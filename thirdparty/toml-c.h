...
int
read_unicode_escape(uint64_t code, char buf[6])
{
...
static int
e_syntax(context_t *ctx, toml_pos_t pos, const char *msg)
{
...
static int
e_keyexists(context_t *ctx, toml_pos_t pos)
{
...
static char *
norm_lit_str(const char *src, int srclen, int *len, bool multiline, char *errbuf, int errbufsz)
{
        const char *sp = src;
...
// in errbuf.
static char *
norm_basic_str(const char *src, int srclen, int *len, bool multiline, char *errbuf, int errbufsz)
{
        const char *sp = src;
        const char *sq = src + srclen;
        char *dst = 0; /// will write to dst[] and return it
...
static char *
normalize_key(context_t *ctx, token_t strtok, int *keylen)
{
...
static int
check_key(toml_table_t *tbl, const char *key, toml_keyval_t **ret_val, toml_array_t **ret_arr, toml_table_t **ret_tbl)
{
        int i;
        void *dummy;
...
static int
key_kind(toml_table_t *tbl, const char *key)
{
...
create_keyval_in_table(context_t *ctx, toml_table_t *tbl, token_t keytok)
{
        int keylen;
...
static toml_table_t *
create_keytable_in_table(context_t *ctx, toml_table_t *tbl, token_t keytok)
{
        int keylen;
        char *newkey = normalize_key(ctx, keytok, &keylen);
        if (!newkey)
...
static toml_array_t *
create_keyarray_in_table(context_t *ctx, toml_table_t *tbl, token_t keytok, char kind)
{
        int keylen;
        char *newkey = normalize_key(ctx, keytok, &keylen);
        if (!newkey)
...
static bool
skip_newlines(context_t *ctx, bool isdotspecial)
{
...
static inline int
eat_token(context_t *ctx, tokentype_t typ, bool isdotspecial, const char *fline)
{
        if (ctx->tok.tok != typ)
...
static int
parse_inline_table(context_t *ctx, toml_table_t *tbl)
{
...
//   key = { table }
static int
parse_keyval(context_t *ctx, toml_table_t *tbl)
{
        if (tbl->readonly)
...
toml_table_t *
toml_parse(char *toml, char *errbuf, int errbufsz)
{
        context_t ctx;
...
void
toml_free(toml_table_t *tbl)
{
        xfree_tbl(tbl);
}

static void
set_token(context_t *ctx, tokentype_t tok, toml_pos_t pos, char *ptr, int len)
{
        token_t t;
        t.tok = tok;
        t.pos = pos;
...
static void
set_eof(context_t *ctx, toml_pos_t pos)
{
...
static bool
scan_offset(const char *p, int *tz)
{
        int sign = p[0];
...
static int
scan_string(context_t *ctx, char *p, toml_pos_t *pos, bool dotisspecial)
{
        char *orig = p;
...
static int
next_token(context_t *ctx, bool dotisspecial)
{
        // Eat this tok.
        char *p = ctx->tok.ptr;
...
const char *
toml_table_key(const toml_table_t *tbl, int keyidx, int *keylen)
{
        if (keyidx < tbl->nkval) {
...
toml_unparsed_t
toml_table_unparsed(const toml_table_t *tbl, const char *key)
{
...
toml_array_t *
toml_table_array(const toml_table_t *tbl, const char *key)
{
...
toml_table_t *
toml_table_table(const toml_table_t *tbl, const char *key)
{
...
toml_unparsed_t
toml_array_unparsed(const toml_array_t *arr, int idx)
{
        return (0 <= idx && idx < arr->nitem) ? arr->item[idx].val : 0;
}

int
toml_table_len(const toml_table_t *tbl)
{
        return tbl->nkval + tbl->narr + tbl->ntbl;
}

int
toml_array_len(const toml_array_t *arr)
{
        return arr->nitem;
}

toml_array_t *
toml_array_array(const toml_array_t *arr, int idx)
{
        return (0 <= idx && idx < arr->nitem) ? arr->item[idx].arr : 0;
}

toml_table_t *
toml_array_table(const toml_array_t *arr, int idx)
{
...
int
toml_value_timestamp(toml_unparsed_t src_, toml_timestamp_t *ret)
{
        if (!src_)
                return -1;
...
// Raw to boolean
int
toml_value_bool(toml_unparsed_t src, bool *ret_)
{
        if (!src)
                return -1;
        bool dummy = false;
...
// Raw to integer
int
toml_value_int(toml_unparsed_t src, int64_t *ret_)
{
        if (!src)
                return -1;
...
int
toml_value_double(toml_unparsed_t src, double *ret_)
{
        if (!src)
                return -1;
...
int
toml_value_string(toml_unparsed_t src, char **ret, int *len)
{
...
toml_value_t
toml_array_string(const toml_array_t *arr, int idx)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_array_bool(const toml_array_t *arr, int idx)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_array_int(const toml_array_t *arr, int idx)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_array_double(const toml_array_t *arr, int idx)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_array_timestamp(const toml_array_t *arr, int idx)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_table_string(const toml_table_t *tbl, const char *key)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_table_bool(const toml_table_t *tbl, const char *key)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_table_int(const toml_table_t *tbl, const char *key)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_table_double(const toml_table_t *tbl, const char *key)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
...
toml_value_t
toml_table_timestamp(const toml_table_t *tbl, const char *key)
{
        toml_value_t ret;
        memset(&ret, 0, sizeof(ret));
